...
A Verification Run or Interactive Run mints one `supply_variable` decision for every missing runtime Variable. Each decision names the Variable and whether it is secret. Supply keeps the literal in the Agent Session process, marks the Variable supplied, and clears that decision. Refusal clears the decision without supplying a value, so the active Step cannot use it and coverage remains incomplete. Decision history and timeline entries retain the Variable name but redact a supplied literal even if the client repeats it in `userMessage`.

A paused Execution Boundary mints a `boundary` decision on its Agent Session rather than in the catalog, because a Run may pause where no draft revision is under review. Allowing one records the same grant a direct Agent View confirmation did: an allowed domain covers that host for the Run, and an allowed confirmation or new objective covers one action attempt. Refusing blocks the action and records the refusal in the timeline. The server refuses a boundary `allow` while the user holds Takeover, as it always has, and still accepts `refuse`, so a user who took control can block the attempt without handing the browser back first. Resolving either way clears the pause, and the resolution joins the session's decision history. Agent View mirrors the paused Boundary read-only.
