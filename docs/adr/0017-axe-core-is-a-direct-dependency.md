...
- **A CSP that forbids inline scripts no longer fails the Audit.** The replaced tool injected `axe.min.js` as a script element, so such a page failed loudly; the wrapper injects through `page.evaluate`, which page CSP does not govern. The page now audits instead — which was always the honest outcome, since a page the engine could not read is not a clean one.
