...
The file is written into a per-session temporary directory created on the first capture and removed when the Agent Session's scope closes, so it lives exactly as long as the session that can still be asked about it. The directory sits under the MCP process's owned resource directory, which the existing startup scavenger already reclaims after a crash. Nothing durable is written. Teaching keyframes travel as content-addressed references in the timeline and are fetched one at a time with `agent_teaching_keyframe_get`. Since #262, that fetch returns the recording's local PNG path instead of inline bytes. The file lasts until verification purges the Teaching Recording.

The Teaching timeline still hides artifact paths. A keyframe path is exposed only after the agent claims the recording and asks for that keyframe by id. A browser screenshot path belongs to the live Agent Session and lasts only until that session closes.
