...
- **A `runtime` Variable is asked for at the Step that references it, not before the Run.** Preflight still resolves everything whose value can be known in advance, and still fails closed on it. A `runtime` Variable's value cannot be known in advance by definition — an OTP does not exist until the Step before it has submitted the form that sends it — so asking up front makes the person answering hold a single-use code while the Run walks to the field, which is how a code expires before it is typed. The only thing that can go wrong late is the person declining, and that fails the Step it belongs to, with its own error, rather than eight Steps later. A Variable two Steps reference is asked for once: Steps execute one at a time, and the answer is recorded where substitution reads it.
