...
A **Run** validates before it opens a browser, aborts and retries on Step failure, classifies why it failed, and persists a self-contained artifact on every path including interruption. These rules apply to headless CLI Runs and, later, to Audit View, because [ADR 0002](./0002-cli-is-the-sole-runner.md) gives both one engine.
...
- **An interrupted Run writes no `run.json`, deliberately.** A Run that was cancelled neither completed nor failed, and `RunOutcome` offers only those two. Recording it as either would be a lie the rest of the system acts on: `runIsBaselineEligible` keys on the outcome, so a half-finished Run would either anchor comparisons it has no business anchoring, or report a failure the site never caused. Artifacts that describe themselves are still written — a video manifest accounts for whatever was captured before the interruption ([ADR 0010](./0010-run-video-is-unredacted.md)) — so nothing is left unexplained on disk. If interrupted Runs later need to be inspected, that wants a third outcome and the Baseline rules to match, not a partial record wearing one of the existing two.
