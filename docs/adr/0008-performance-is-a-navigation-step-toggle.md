...
- A metric the page did not produce is recorded as absent, never as zero, so the fastest possible page stays distinguishable from an unmeasured one. An interaction too fast to be reported as an `event` entry still yields an INP, for the same reason.
