...
After Teaching, Agent View presents the draft's Agent Steps, Variables, Domain Scope, and Confirmation Steps. A direct user action may authorize exactly one Verification Run. After successful verification, a second direct user action may approve the Agent Flow Revision; the external agent can request both actions but cannot invoke them itself.
