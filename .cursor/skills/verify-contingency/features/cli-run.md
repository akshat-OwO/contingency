...
- `run-complete` finishes a one-step navigate Flow against the disposable fixture page.
...
- `control-contingency fixture start` has printed `flow=` pointing at `$CONTINGENCY_VERIFY_DIR/verify-flow.json`.
- Chromium for `playwright-core` is installed.

- **Start fixture.** Serve the disposable page. Run `control-contingency fixture start`. Stdout contains `fixture=http://127.0.0.1:` and `flow=` ending in `verify-flow.json`.
...
- **Confirm artifacts.** Read `run.json` in that directory. `flow.title` is `Verify fixture` and `outcome` matches the printed line. The directory is not under `~/.local/state/contingency`.
...
- The fixture process is recorded on `instance.json`. `cleanup` kills it. `fixture stop` kills it and leaves the CLI running.
