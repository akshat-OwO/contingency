...
- [Agent View](./agent-view.md) covers the unavailable state on `web`, the empty state when MCP has no session, and a live session's Takeover: the browser toolbar, the canvas, and the action timeline.
