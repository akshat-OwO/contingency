...
- [Agent View](./agent-view.md) covers the unavailable state on `web`, the empty state when MCP has no session, same-document action snapshots, a live session's Takeover, Teaching and draft compilation, and evidence-backed per-Step Verification Run verdicts.
