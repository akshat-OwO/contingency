...
- A live Interactive Run's dock names the Flow Skill, the active Agent Step and its `Done when:` line, and coverage as `<executed> of <total> Agent Steps executed`. It offers `Extend Agent Step ceiling` and `Extend Run ceiling` until the Run ends; those buttons are the only way either ceiling grows. A live Dry Run's dock reads `Rehearsing the flow skill <name>.` and names its changed inputs.
