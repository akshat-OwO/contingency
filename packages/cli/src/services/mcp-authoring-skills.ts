...
const learnFlowSkillPrompt = [
  ...
  "7. Ask for ordinary inputs again and change at least one demonstrated input when the task permits it. Start a fresh-context Dry Run with agent_flow_skill_dry_run_start. For a secret input pass its name with secret:true and no value; ask the user to supply it in the returned Workspace, then use its uppercase Variable name with agent_variable_enter. Follow the returned Flow Skill files and assess each numbered Agent Step against its Done when line with agent_run_step_assess. The Dry Run ends when its Steps end; its result is derived from their Assessments.",
  "8. A failed report keeps the Teaching Recording and this claim. Fix the package and call agent_flow_skill_save with the same claim operation id, then start another Dry Run.",
  '9. A passing report keeps the Teaching Recording. Show the result to the user. Call agent_flow_skill_decide with decision "verify" only after the user explicitly chooses Verify flow, or with decision "reject" when the user rejects it.',
].join("\n");
